...
# ---
# +
# Feature matching experiment with mfcc features
import argparse
...
# Full feature extraction with MFCCs
...
labels_mfcc_1 = [f"{labels_out[2]}_C{i + 2}" for i in range(12)]
labels_mfcc_1 = [f"{labels_out[3]}_C{i + 2}" for i in range(12)]

labels = labels_out[:2] + labels_mfcc_1 + labels_mfcc_1
...
def anchor_swap_experiment():
    ...
    for i in range(len(dataset)):
        ...
        output = Path(
            f"{out_dir}/anchor_swap_{Path(data_path).stem}_{i}_e{error:.2f}.png"
        )
    ...
    output = Path(f"{out_dir}/anchor_swap_{Path(data_path).stem}_concat.wav")
...
def scale_experiment():
    ...
    for i in range(len(scale)):
        ...
        output = Path(
            f"{out_dir}/target_scale_{Path(data_path).stem}_{scale[i]:.2f}_e{error:.2f}.png"
        )
        fig = plot_features(y_feat, y_hat_feat, idx, extractor)
        fig.savefig(output, dpi=150)
        plt.close(fig)
    ...
    output = Path(f"{out_dir}/target_scale_{Path(data_path).stem}_concat.wav")
    torchaudio.save(output, audio_tensor, sample_rate)
...
def inverse_scale_experiment():
    ...
    for i in range(len(scale)):
        ...
        output = Path(
            f"{out_dir}/target_inv_scale_{Path(data_path).stem}_{scale[i]:.2f}_e{error:.2f}.png"
        )
        fig = plot_features(y_feat, y_hat_feat, idx, extractor)
        fig.savefig(output, dpi=150)
        plt.close(fig)
    ...
    output = Path(f"{out_dir}/target_inv_scale_{Path(data_path).stem}_concat.wav")
    torchaudio.save(output, audio_tensor, sample_rate)
...
def timbre_only_scale_experiment():
    ...
    for i in range(len(scale)):
        ...
        output = Path(
            f"{out_dir}/timbre_target_scale_{Path(data_path).stem}_{scale[i]:.2f}_e{error:.2f}.png"
        )
        fig = plot_features(y_feat, y_hat_feat, idx, extractor)
        fig.savefig(output, dpi=150)
        plt.close(fig)
    ...
    output = Path(f"{out_dir}/timbre_target_scale_{Path(data_path).stem}_concat.wav")
    torchaudio.save(output, audio_tensor, sample_rate)
