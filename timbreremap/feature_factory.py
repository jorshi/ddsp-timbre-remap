...
def simple_spectral_factory(sample_rate: int):
    return FeatureCollection([SpectralCentroid(sample_rate=sample_rate), SpectralFlatness()])
...
def spectral_loudness_factory(sample_rate: int, num_samples: int):
    ...
    extractor = CascadingFrameExtactor([centroid, loudness], num_frames=[num_frames,], frame_size=frame_size, hop_size=hop_size, include_diff=True)
    return extractor
